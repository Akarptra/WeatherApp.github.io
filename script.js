const API = "6e9e9bc3fd9bd9292b94faef813f7f08";
...
btnEl.addEventListener("click", (e) => {
  ...
  if (inputEl.value !== "") {
    ...
    // Panggil fungsi pencarian berdasarkan nama kota
...
// Call getLocation function directly to get user's location automatically
getLocation();
...
async function findLocationByCoordinates(latitude, longitude) {
  ...
  listContentEl.innerHTML = "";
  try {
      const API_URL = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${API}`;
      const data = await fetch(API_URL);
      const result = await data.json();
      ...
      if (result.cod !== "404") {
          ...
          // Change background based on location name
          changeBackground(result.name);
...
// Function to change background based on location name
function changeBackground(locationName) {
  document.body.style.backgroundImage = `url("https://source.unsplash.com/1600x900/?${locationName}")`;
}
